...
for pileupcolumn in samfile.pileup(args.chr, args.start , args.start+1 ):  #pysam is 0-based, but human minds are not
    for pileupread in pileupcolumn.pileups:
        if pileupread.is_del and not pileupread.is_refskip and pileupread.indel==0 and (pileupcolumn.pos >= args.start ) and (pileupcolumn.pos <= args.start):
            print ('%s\t%s\t%s\t%s\t%s\t%s' %
                 (args.chr,
                  ...
                  "*"))     
        if not pileupread.is_del and not pileupread.is_refskip and pileupread.indel==0 and (pileupcolumn.pos >= args.start ) and (pileupcolumn.pos <= args.start  ):
           print ('%s\t%s\t%s\t%s\t%s\t%s' %
                 (args.chr,
                  pileupcolumn.pos +1,
                  fastafile.fetch(args.chr,pileupcolumn.reference_pos ,pileupcolumn.reference_pos +1),  
